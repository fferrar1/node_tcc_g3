...
module.exports = {
    async listarPedidos(request, response) {
        try{
            const sql = 'SELECT pdd.pedido_quant, pdd.pedido_valor_produto, cmp.compra_id, cmp.compra_status, pdt.prod_cod_barras, pdt.prod_descricao, pdt.prod_nome, pdt.prod_marca, pdt.prod_valor, pdt.prod_img, pdt.prod_estoque FROM pedidos pdd INNER JOIN compra cmp ON pdd.compra_id = cmp.compra_id INNER JOIN produtos pdt ON pdd.prod_id = pdt.prod_id';
